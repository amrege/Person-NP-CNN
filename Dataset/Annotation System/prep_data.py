...
def parse_frames(sensor, data_dir, fpv):
    ...
    '''
    out_dir = os.path.join('.', sensor, 'd_parsed')
    print 'Parsing data...Output to:', out_dir
    if os.path.exists(out_dir): rmtree(out_dir)

    frame_dir = os.path.join(data_dir, sensor, 'home/station/ipCamDetection_v2/bin/output/cvpr', 'd')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', default='/data/files0930',
                        help='Root directory where all data resides')
    parser.add_argument('--out_dir', default='./video',
